...
def main(input_folder:Path=typer.Argument(...,help="Input Folder That Contain Original Json File")
         ,keys:str=typer.Argument(...,help="Keys That you want To clean")
     ...
     with Progress() as p:
          ...
          for i in files:
            ...
            #    print(input_file)
               ...
               update_data=clean_data(data,keys)
...
def main(file:Path=typer.Argument(...,help="Pass Json File"),keys:str=typer.Argument(...,help="Keys that you wantg to clean...")):
    keys=keys.split(",")
    data=read_json(file)
    upadted_data=clean_data(data,keys)
